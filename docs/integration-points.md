...
`hardware/init.py`가 위치로 초기화만 하고 있음(2026-07-27 실물 재보정으로 값 자체는 1016으로 갱신됨). 실물 로봇으로 어떤 관절인지 확인되면 `hardware/config.py`에 정식 이름을 붙이고 "용도 불명" 주석을 지운다.
...
- **`subtitle.py`의 번들 폰트가 런타임에 자동 등록 안 됨**: `display/fonts/`에 파일은 있지만 tkinter가 시스템에 설치된 폰트만 찾는다. 자막 기능 자체가 필요 없어져서(2026-07-27 사용자 결정) 더 이상 안 고쳐도 됨.
