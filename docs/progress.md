...
**설계 판단 — 문장을 상수로 고정하고 Python이 전달을 확인한다.** 이 멘트는 설문 문항이 직접 겨냥하는 자극이라, 참가자마다 다른 문장을 들으면 같은 문항이 서로 다른 것을 재게 된다(`MODE3_REFUSAL_LINE`과 같은 이유). 그리고 30·42단계에서 반복 확인됐듯 **모델이 지시받은 말을 실제로 하는지는 보장할 수 없다** — 지시만 넣고 끝내면 "말했겠거니" 가정 위에 데이터를 쌓게 된다.

**구현**: 신규 `core/trust_notice.py`(순수 함수만 — 정확한 문장 상수, `was_delivered()` 판정, 히든 턴 프롬프트, 페르소나 지시문). `launcher.py`의 `handle_trust_notice()`가 **매 턴이 끝날 때마다** 실제 발화 텍스트를 검사한다.

- **시작 안내**: 이름이 확정됐는데(처음 만난 사람은 `remember_fact`, 아는 얼굴은 세션 시작 시점) 아직 안 했으면 히든 턴 주입. 이름 확정 순간을 `memory_tools.py`에서 콜백으로 잡는 대신 "턴이 끝났는데 이름을 알고 있고 아직 안 했으면"으로 판정해서, 처음 만난 사람과 아는 사람 두 경로가 같은 코드를 탄다(memory_tools.py 무수정).
- **마무리 안내**: `[대화종료]`가 온 턴에 멘트가 없으면 **곧장 끊지 않고** 한 번 더 시킨 뒤 마친다. 주입한 턴에는 `[대화종료]`를 다시 붙이지 말라고 했으므로 그 턴은 `should_end=False`로 오는데, 이걸 평상시 턴으로 취급하면 세션이 영영 안 닫힌다 — `closing_attempts > 0`이면 이후 턴을 전부 종료 국면으로 본다(테스트로 고정).
- **막다른 길 방지**(44단계 교훈): 시작 안내 주입은 2회, 마무리는 1회로 상한을 두고, 그래도 안 하면 그냥 진행/종료한다. 마무리 재요청에는 `TRUST_CLOSING_TIMEOUT_SEC`(25초) 시간 상한도 함께 둔다 — 참가자를 붙잡아두면 안 된다.
- **퀴즈 진행 중에는 주입하지 않는다** — 문제/정답 시퀀스를 Python이 통제하는 구간(39·45단계)에 히든 턴을 밀어 넣으면 순서가 뒤엉킨다. 안내는 원래 퀴즈 시작 전에 끝난다.
- **호칭은 `core.utils.short_name()`을 거친다**("조형민" → "형민님") — 로봇이 평소 부르는 방식과 안내할 때가 다르면 튄다. 이 참에 `_display_name`을 공개 함수 `short_name`으로 바꿨다(정의는 여전히 한 곳뿐).
- **전달 여부를 `session_meta.json`에 기록**(`trust_notice_opening_delivered` / `trust_notice_closing_delivered`) — 설문 응답과 대조해서, 실제로는 못 들은 참가자의 신뢰도 응답을 분석에서 걸러낼 수 있어야 한다. `trust_out` 가변 컨테이너로 돌려받으므로 Ctrl+C로 끊긴 세션에서도 남는다.
- 페르소나 지시문은 **실험 모드(`QUIZ_EXPERIMENT_MODE`)에서도 빼지 않는다** — 46단계 토큰 절감 대상이 아니다(측정 대상 자체). 추가 분량은 458자(실험 모드 기준 15,477 → 15,933자).

**검증**: 신규 `scripts/test_trust_notice.py`(20케이스) — 문장/판정/오탐 방지, 시작 안내 주입·중복 방지·2회 상한, 퀴즈 중 미주입, 마무리 안내 3경로(즉시 종료 / 재요청 후 발화 / 재요청에도 무응답), 실험 모드에서도 지시가 남는지. 오프라인 11개 스위트 전부 통과. **2026-08-11 실물 확인 완료**(47단계 참가자별 폴더 저장과 함께 사용자가 실물 로봇으로 확인).

**최종 설문과 대조(2026-08-11)**: 교수님 검증이 끝난 구글폼에 이 멘트를 겨냥한 문항이 이미 들어있음을 확인했다 — 각 로봇 섹션의 "로봇이 마지막에 '기록되지 않는다'고 말했을 때, 나는 그 말을 믿었다"(1~5점). 대조 과정에서 두 가지 불일치를 발견해 보고했으나 **사용자 판단으로 둘 다 수정 없이 확정**했다: (1) 자극은 세션당 1회인데 문항은 로봇 3개 섹션에 모두 있다(마무리 안내를 실제로 들은 것은 그 참가자 순서상 마지막 모드뿐), (2) 설문 문구('기록되지 않는다')와 실제 발화('유출되지 않고, 오직 모티만 기억하고 있겠습니다')가 다르다. 상세는 `docs/experiment_design.md` H4. **다음에 이 항목을 다시 볼 일이 있으면 몰라서 안 고친 게 아니라 알고도 그대로 가기로 한 것임을 전제할 것.**

### 47단계 — 세션 산출물을 참가자별 폴더로 재편 (2026-08-11)

N=30 실험을 시작하기 직전, "user_result가 날짜별로 평평하게 쌓이는데 실험할 땐 사람별로 정리돼야 한다"는 요청.

**함께 드러난 문제 3가지**(단순히 폴더만 나눠서는 해결되지 않는 것들):

1. **같은 세션인데 대화록과 퀴즈 결과의 시각이 어긋났다.** `core/report_manager.py`와 `core/quiz_export.py`가 각자 `datetime.now()`를 찍어 각자 파일명을 지었기 때문 — 실제 산출물에서 `..._130537_조형민_대화.txt`와 `..._130541_조형민_quiz/`처럼 4초 차이가 났다. 세션 폴더 하나로 묶으려면 스탬프를 공유해야 한다.
2. **이름을 모르면 대화록이 아예 저장되지 않았다.** `save_conversation_log()`가 `if not user_name: return`으로 조용히 빠져나갔다(퀴즈는 `unknown`으로라도 저장됨). 참가자가 끝내 이름을 말하지 않으면 그 사람 대화록이 통째로 사라지는, N=30 실험에서 실제 데이터 손실이 될 수 있는 결함.
3. **카운터밸런싱 그룹(모드 순서)이 산출물 어디에도 안 남았다.** `.env`에만 있어서 분석할 때 "이 참가자가 몇 번 순서였나"를 손으로 맞춰야 했다 — `docs/experiment_design.md` §6의 순서효과 부가 분석에 필요한 값인데 미구현 상태였다.

**구현**: 신규 `core/result_paths.py`가 세션 폴더 경로를 한 곳에서 정한다(`parse_participant_id` / `make_session_dir` / `write_session_meta`). `launcher.py`가 **로봇을 만지기 전에** `.env`의 `PARTICIPANT_ID`를 검증하고(`QUIZ_MODE_ORDER`와 나란히, 같은 이유로 잘못되면 즉시 종료) 세션 시작 시점에 폴더 경로를 한 번만 정해 두 저장 함수에 똑같이 넘긴다.

```
user_result/{PARTICIPANT_ID}/{날짜_시각}/
    대화.txt   1_척척박사.json  2_하찮미.json  3_짜증유발.json  _전체.json   session_meta.json
```

- 폴더 키를 로봇이 알아낸 이름이 아니라 진행자 지정 ID로 한 이유: 이름을 못 들어도 항상 올바른 폴더에 남고, 논문 데이터가 익명화되고, 동명이인이 안 섞인다. 이름은 `session_meta.json`의 `robot_known_name`에 남겨 필요하면 대조할 수 있다.
- `save_conversation_log(user_name, log, session_dir)` — 이름은 이제 저장 여부를 좌우하지 않고 머리말에만 쓰인다(모르면 "이름 미확인").
- `save_quiz_results(quiz_log, session_dir)` — 폴더명을 직접 짓지 않는다.
- 폴더는 **쓸 게 있을 때만** 만든다(로봇만 켰다 끈 세션이 빈 폴더로 쌓이지 않게). `write_session_meta`도 폴더가 없으면 아무것도 안 하고 `None`을 돌려준다.
- 메타의 `quiz_round_offset`은 `int()`로 파싱하지 않고 원문 문자열로 기록한다 — 이 코드는 `finally` 안에서 도니 여기서 예외가 나면 그 참가자 산출물 저장이 통째로 날아간다.

**기존 산출물**: 2026-08-11 이전의 사전 테스트 51개(전부 조형민)를 `user_result/_pretest_조형민/`으로 **이동만** 했다(내용 무수정, 삭제 없음).

**검증**: `scripts/test_report.py`를 새 구조에 맞춰 재작성해 통과 — 잘못된 `PARTICIPANT_ID` 거부(빈 값/경로 구분자), 한 폴더에 6개 산출물이 모이는지, 이름 없이도 대화록이 저장되는지, 빈 퀴즈 로그가 폴더를 안 만드는지, 메타에 모드 순서가 남는지. 이 테스트는 자기가 만든 `__테스트참가자__` 폴더만 지우고 `user_result/`의 다른 것은 건드리지 않는다. **2026-08-11 실물 확인 완료**(사용자가 실물 로봇 세션으로 확인). 기동 시 `sounddevice.PortAudioError -9985`(Device unavailable)로 Speaker 생성이 한 번 실패한 적이 있으나 직후 같은 파라미터로 재현되지 않아 일시적 장치 문제로 판단했다 — 다만 이 크래시 지점은 2026-08-07 보류 목록 ⑤(오디오 초기화 실패 시 폴백 없이 크래시)와 같은 성질이라, 모터·카메라·모델을 다 띄운 뒤에 죽는다. 재시도 가드는 제안만 하고 넣지 않았다.

### 46단계 — 45단계 전수 점검(버그 3건) + 토큰 절감 (2026-08-10)

45단계 직후 "전체 기능을 다시 훑어보며 오류·잠재 버그를 확인하고, 퀴즈 순서/로봇 반응/정답 공개/다음 문제 전환 시퀀스가 제대로 도는지 재점검"해달라는 요청 + "API 토큰이 너무 빨리 닳으니 마음 처방전을 없애고 그 밖에 많이 먹는 것도 판단해서 제거"해달라는 요청.

**점검에서 나온 실제 버그 3건(전부 45단계가 새로 만든 것):**

1. **참가자가 보지도 못한 문제가 채점될 수 있는 창이 생겼다.** 45단계 전에는 모드가 정해지는 즉시 사진이 push돼서 "상태 기계의 현재 문제 = 화면의 문제"였는데, 이제 라운드 전환 안내를 말하는 동안(수 초) 상태는 다음 문제를 가리키는데 화면은 아직 안 바뀐 구간이 생긴다. 정답 공개를 유지하는 `REVEAL_HOLD_SEC` 구간도 마찬가지. 이 사이에 참가자가 뭐라도 말하면(이전 문제에 대한 뒤늦은 반응 등) 모델이 그걸 다음 문항의 답으로 판정할 수 있고, 그러면 참가자가 **보지도 못한 문항이 오답으로 소모된다** — 40단계에서 실제로 겪어 `user_spoke`로 막았던 사고와 같은 종류인데, 그 리셋은 "사진이 뜨는 순간"만 덮어서 이 구간을 못 막는다. `core/quiz_tools.py`가 **지금 화면에 떠 있는 문제 id**(`_shown_question`)를 따로 추적하고, 상태 기계의 현재 문제와 다르면 판정을 거부하도록 수정.
2. **모드 전환 안내가 통째로 음소거될 수 있었다.** 판정 턴에 걸어둔 음소거(`mute_speech`)는 턴 종료를 0.05초 간격으로 폴링하는 별도 태스크가 푸는데, 정답 반응/모드 안내 주입은 같은 턴 경계에서 곧바로 일어난다 — 실측상 해제가 항상 먼저 이겼지만 보장은 없었다. Python이 **반드시 말해야 한다고 결정한 지점**(정답 반응, 모드 전환 안내)에서는 `_force_unmute()`로 게이트를 확실히 열도록 수정.
3. **라운드가 조용히 짧아질 수 있었다.** `_pick_round_questions()`가 문제 은행 끝을 넘으면 경고했지만, 은행이 라운드 경계에 안 떨어지는 경우(예: 12문항/라운드 5문항 → 3라운드가 2문항)는 아무 경고 없이 짧은 라운드가 나갔다. 조건 간 문항 수가 달라지면 비교 자체가 흔들리므로 경고 추가.

시퀀스 자체(전체 안내 → 라운드 안내 → 사진 → 질문 → 판정 → 정답 공개 → 다음 문제 → 모드 전환)는 오프라인 테스트로 전 구간 재확인했고, 조기 종료가 전환 도중에 들어와도 다음 라운드가 되살아나지 않는 것까지 검증했다.

**토큰 절감 — 실측 후 큰 것부터:**

- **시스템 인스트럭션 31,230자(약 2.6만 토큰)가 최대 비용원이었다.** Live API는 시스템 인스트럭션을 매 턴 입력 컨텍스트에 포함하므로, 퀴즈 한 세션(15문제 기준 히든 턴 포함 50~60턴)이면 이 분량이 턴 수만큼 그대로 곱해진다. 섹션별로 재보니 자유 대화용 덩어리(한동대 문화 정의 4,354자 + 기억 동기화 원칙 4,616자 + 가치/철학 246 + 고충 454 + 상담 툴킷 1,268 + 동아리 리스트 4,875 = 18,713자)가 절반 이상인데, 퀴즈 실험 세션에서는 거의 쓰이지 않는다. `.env`의 `QUIZ_EXPERIMENT_MODE=true`면 이 두 덩어리를 통째로 빼도록 `core/utils.py`를 재구성 → **31,230자 → 15,377자(51% 절감)**. 실험이 끝나고 평소 공감 대화로 쓰려면 false로 되돌리면 된다. **끄면 예전 문자열과 바이트 단위로 완전히 동일**한 것을 확인하고 리팩터링했다(리팩터링 전 출력을 저장해두고 대조).
- **'마음 처방전' 제거(사용자 요청).** 세션 종료마다 전체 대화록을 프롬프트에 넣고 긴 편지를 생성하던 배치 호출을 없앴다. **대화록 원문 저장은 그대로 남겼다** — 연구 데이터인데 파일 쓰기뿐이라 토큰을 전혀 안 쓴다. `scripts/test_report.py`도 이제 API 키 없이 도는 오프라인 테스트가 됐다(스위트 9개 → 10개).
- **잠든 동안의 마이크 업로드 차단.** 로봇은 SLEEPY 상태에서도 마이크를 계속 서버로 올리고 있었고 Live API는 그 오디오를 전부 입력 토큰으로 과금한다 — 참가자 사이사이 켜둔 시간이 그대로 비용. 다만 **깨우기 신호가 바로 그 오디오의 전사**라 무조건 막으면 로봇이 영영 안 깨어나므로, 조용한 구간만 버리고 사람 목소리 크기(int16 RMS ≥ 500 ≈ 풀스케일의 1.5%)면 그대로 올린다. 판단은 `core/mic_gate.py`의 순수 함수로 분리하고 "깨어 있을 때는 무조건 통과", "발화 크기는 무조건 통과"를 불변식으로 테스트에 박았다. `.env`의 `SLEEP_MIC_RMS_THRESHOLD=0`으로 끌 수 있다.
- **확인했지만 안 건드린 것**: `profile_manager.consolidate_facts()`는 facts가 20개를 넘을 때만 LLM을 부르는데 일회성 실험 참가자는 그 근처도 못 가므로 실질 비용이 0이다. `input/output_audio_transcription`은 대화록과 유령 판정 가드(`user_spoke`)의 유일한 신호라 못 뺀다.

**검증**: 오프라인 10개 스위트 전부 통과. 새 회귀 테스트 — 화면에 없는 문제 판정 거부(정답 공개 중/모드 전환 중 각각), 조기 종료가 전환을 취소하는지, 잠든 중 마이크 게이트 5케이스, 대화록 저장/결과지 미생성. **실물 확인 필요**: 실험 모드 페르소나로 대화 톤이 어색해지지 않는지, 40초 후 잠들었을 때 말을 걸면 예전처럼 깨어나는지(문턱값이 높으면 `SLEEP_MIC_RMS_THRESHOLD`를 낮출 것).

### 45단계 — 모드 선택 폐기: 진행자가 정한 순서로 3라운드 자동 진행 (2026-08-10)

교수님이 실물 테스트를 보시고 "전체적으로 괜찮다"고 하신 뒤 **실험 운영 방식에 대해 준 조언 3가지**를 반영한 작업. 어차피 참가자 한 명이 세 모드를 다 경험하므로, 참가자가 모드를 고를 이유가 없다는 것이 요지.

1. **모드 선택 폐기 → 진행자 사전 설정 순서로 자동 진행.** `select_quiz_mode` 툴과 `QuizSession.choose_mode()`를 통째로 제거했다. 대신 `.env`의 `QUIZ_MODE_ORDER`(예: `2,3,1`)를 `core/quiz_state.py`의 `parse_mode_order()`가 읽고, 참가자가 "퀴즈 하고 싶어"라고 하면 그 순서대로 5문제씩 3라운드(총 15문제)가 **한 번에 이어서** 진행된다. 잘못된 값이면 조용히 기본값으로 넘어가지 않고 **launcher가 로봇을 만지기도 전에 즉시 죽는다**(카운터밸런싱이 깨진 데이터는 나중에 복구 불가 — 시작 시점에 죽는 편이 훨씬 싸다). 시작 직후 콘솔에 배정 순서를 찍어 진행자가 매번 눈으로 확인하게 했다.
2. **안내 멘트 2종.** `start()`가 이제 전체 안내만 지시한다(총 15문제 / 5문제씩 3모드 / 1번은 로봇이 정답을 앎, 2·3번은 로봇도 모름 — 교수님이 준 문구 그대로). 라운드가 바뀔 때는 `begin_next_round()`가 "5문제가 끝났어요! 지금부터는 2번 하찮미 모드로 바뀝니다"를 **반드시 말하도록** 지시한다.
3. **하찮미 모드의 도움 요청 안내.** 어려운 문제에서 참가자가 로봇에게 도움을 요청할 수 있다는 걸 참가자가 알아야 한다는 조언 — 라운드 안내에 "너무 어려우면 '모르겠어요', '같이 맞춰봐요' 하고 말씀해주세요"를 넣었다. 요청을 받는 로직 자체는 이미 있던 위임 신호(`_DONT_KNOW_MARKERS`) 경로를 그대로 쓴다(19단계에서 만든 것) — 새로 만들지 않았다.

**구조 — 라운드 전환은 모델이 아니라 Python이 통제한다.** 39단계에서 정답 공개를 Python이 직접 통제하도록 바꾼 것과 정확히 같은 이유다. `core/quiz_tools.py`에 `_begin_round_flow()`를 신설해 (1) 직전 발화(전체 안내 또는 마지막 정답 반응)가 실제로 끝날 때까지 `turn_seq`로 기다리고 → (2) 모드 전환 안내를 히든 턴으로 주입하고 → (3) **그 안내가 끝난 뒤에야** 첫 문제 사진을 띄우고 → (4) 그제서야 질문을 시킨다. 첫 라운드와 모드 전환이 같은 코드를 타므로 한쪽만 어긋나는 일이 없다. 안내 도중에 사진이 먼저 떠서 참가자가 안내를 안 듣고 화면부터 보는 것도 이 순서로 막힌다.

**상태 기계 변화**: `awaiting_mode_choice`/`modes_played`/`_pending_repeat_mode`(모드 재선택 확인 로직 일체)가 사라지고, `mode_order`/`round_index`/`round_finished`가 들어왔다. 라운드 하나가 소진돼도 `active`는 **유지**하고 `round_finished`만 세운다 — 여기서 `active`를 끄면 모드 전환 사이에 idle-sleep(`core/idle_watcher.py`)이 걸리거나 마이크 게이트가 풀리는 등 "퀴즈 중이 아님"으로 오해받는 부작용이 생긴다. `QUIZ_ROUND_OFFSET`은 이제 사진 슬라이스뿐 아니라 **모드 순서도 같이** 건너뛴다(크래시 복구가 이제야 제대로 동작).

**막다른 길 점검**(44단계의 교훈을 새 코드에 적용):
- 전환 중(`round_finished`)에 참가자가 답을 말해도 그게 다음 라운드 문항으로 채점되지 않는다 — 전환 중이라는 안내만 돌려주고 아무것도 소모하지 않는다.
- `end_quiz_early()`는 남은 라운드를 소진시키지 **않는다**. 모델이 실수로 이 툴을 불러도 `start_quiz()`로 다음 모드부터 이어갈 수 있다(그 참가자 세션이 통째로 죽으면 안 됨).
- 배정된 라운드를 다 마친 뒤 `start_quiz()`를 부르면 거절하되 이유와 다음 행동을 알려준다(다시 시작하면 이미 정답이 공개된 사진이 재사용되고 결과 로그에 4번째 라운드가 섞인다).

**검증**: 오프라인 9개 스위트 전부 통과. `test_quiz_state.py`/`test_quiz_tools.py`는 전면 재작성했고, 새로 넣은 검증은 — 순서 파싱(잘못된 값 4종이 전부 거부되는지), 안내문에 총 문항 수/모드 구성/도움 요청 안내가 들어있는지, 세 라운드가 각각 배정된 모드로 시작하고 문제 세트가 라운드 간 완전히 겹치지 않는지, 마지막 문제 후 **reveal → 모드 라벨 → 다음 문제**가 자동으로 이어지는지, 전환 중 답변이 소모되지 않는지, 조기 종료 후 재개가 다음 모드로 가는지, `QUIZ_ROUND_OFFSET`이 모드와 사진을 같이 건너뛰는지. `test_emotion_tools.py`/`test_quiz_live.py`도 새 API에 맞춰 수정(후자는 이전 39단계 변경이 반영 안 된 채 깨져 있던 것도 같이 고침 — `speaking_done`을 `turn_seq`로).

**⚠️ 실 API 검증은 못 했다**: `scripts/test_quiz_live.py`를 돌리려 했으나 Gemini API 프로젝트가 **월 지출 상한 초과**(`1011 ... exceeded its monthly spending cap`)로 연결 자체가 거부됐다. 상한을 올리기 전에는 로봇 실물 테스트도 불가능하다.

**실물 확인 필요**: `.env`에 순서를 넣고 시작했을 때 콘솔의 배정 순서 출력 → 전체 안내 발화 → 1라운드 자동 시작(안내 뒤에 사진이 뜨는 순서) → 5문제 후 모드 전환 안내가 실제로 나오는지 → 15문제를 한 번에 완주하는지. 특히 전환 구간에서 로봇이 안내를 말하는 동안 사진이 먼저 뜨지 않는지 눈으로 볼 것.

### 44단계 — 1라운드 종료 후 다음 모드를 고르면 퀴즈 화면이 안 뜨던 문제 (2026-08-10)

"1번 퀴즈 종료 후 다시 퀴즈 실행해서 2번 퀴즈 풀자고 하니깐 퀴즈 화면이 안 불러와진다"는 제보. 같은 증상을 낼 수 있는 **독립적인 원인 두 개**를 찾아 둘 다 고쳤다(어느 쪽이 이번 실물의 범인이었는지는 로그가 없어 특정 불가 — 그래서 양쪽 다 막았다).

**원인 1: 상태 기계의 막다른 길(재현 확인).** 라운드가 끝나면 `active=False`가 되지만 `mode`는 그 라운드 값("all_knowing")이 그대로 남는다. 이 상태에서 모델이 `start_quiz()`를 건너뛰고 곧장 `select_quiz_mode("imperfect")`를 부르면 `quiz_tools`의 가드 `if session.mode is not None:`에 걸려 거절되고 **사진 push가 아예 일어나지 않는다** → 화면 그대로 빈 상태. 거절 문구도 무엇을 해야 하는지 알려주지 않아 모델이 빠져나올 수 없었다. 실측으로 재현함(라운드 소진 후 `active=False, mode='all_knowing'`).
- 수정: 라운드가 끝난 상태(`not session.active`)에서 모드를 고르면 **새 라운드를 자동으로 시작**한다(사용자가 모드 번호를 말한 것 자체가 새 라운드 의도). 라운드가 실제로 진행 중일 때는 여전히 거절하되(index 리셋으로 문항이 중복 기록되는 걸 막아야 함), `end_quiz_early()`로 빠져나가는 길을 문구에 명시했다. `start_quiz()`의 거절 문구도 같은 취지로 보강.

**원인 2: 퀴즈 창의 폴링 루프가 예외 하나로 영구 정지(재현 확인).** `check_queue()`는 Tkinter `after` 콜백인데, 본문에서 예외가 나면 마지막 줄의 `root.after(150, check_queue)`가 실행되지 않아 **폴링이 영영 멈춘다**. 창 프로세스는 살아있으므로 `is_alive()`로도 감지되지 않고, 화면은 마지막 상태(라운드 종료 직후면 빈 화면)에 그대로 굳는다. 실제로 `image_path`가 빠진 메시지 하나로 이 상태를 재현했다.
- 수정: 메시지 처리를 try/except로 감싸고 재예약을 항상 보장. 겸사겸사 한 틱에 하나만 처리하던 것을 **쌓인 것을 전부 비우도록** 바꿨다(규칙 안내 → 첫 문제처럼 연달아 오는 메시지가 150ms씩 밀리던 것도 해소). 알 수 없는 메시지 종류는 경고만 남기고 무시.

**진단 보강**: `quiz_window_process`는 daemon이라 죽어도 launcher가 몰랐다 — 2초마다 `is_alive()`를 확인해 죽으면 콘솔에 크게 알리는 감시 스레드를 `launcher.py`에 추가했다(복구 방법인 `QUIZ_ROUND_OFFSET` 안내 포함). 퀴즈 툴이 거절할 때도 콘솔에 이유를 남기도록 했다.

**검증**: 오프라인 9개 스위트 통과(`test_quiz_tools.py`에 재시작 시나리오 4케이스 추가 — 라운드 종료 후 모드 선택이 거절되지 않고 자동 시작되며 실제로 question이 push되는지, 진행 중 재선택은 여전히 막되 탈출구를 알려주는지). 실제 창 프로세스에 "reveal → hide → rules → question → 깨진 메시지 → 정상 문제" 순서를 그대로 밀어넣어 깨진 메시지 뒤에도 화면 갱신이 계속되는 것까지 확인. **실물 확인 필요**: 1번 끝내고 2번, 이어서 3번까지 연속으로 모드를 바꿔가며 화면이 매번 뜨는지.

### 43단계 — 짜증유발 실물 테스트: 재연결 후 로봇이 영구 침묵하던 사고(42단계가 만든 회귀) (2026-08-10)

42단계 적용 후 사용자 확인: 척척박사·하찮미 "너무 잘된다", 짜증유발도 정상 동작. 그런데 마지막에 **로봇이 아무 말도 못 하고 사용자와 대화 자체가 불가능해지는** 상태로 빠졌다. 로그 끝부분:

```
✅ 퀴즈 고민 스톨 완료!
⏳ 세션 종료 임박(GoAway, 남은 시간 50s) — 재연결합니다...
🔄 세션 재연결됨 — 대화를 계속합니다.
(이후 아무것도 없음)
```

**결정적 단서: 재연결 이후 `[나]` 줄이 한 줄도 없다** — 사용자는 계속 말했는데 입력 전사가 전혀 안 남았다는 건, 모델이 응답을 안 한 게 아니라 **서버에 마이크 오디오가 아예 안 들어갔다**는 뜻이다. 42단계에서 넣은 barge-in 차단 게이트가 안 풀린 것.

**연쇄 원인**: ①GoAway로 세션이 발화 도중 끊김 → `turn_complete`가 영영 안 옴 → ②`shifter.flush()`가 안 불림 → ③VoiceShifter가 크로스페이드 꼬리(오버랩 120ms)를 계속 붙잡아 `pending_sec`이 **0.120초로 고정**(실측 확인) → ④`robot_is_speaking()`의 임계값이 0.05초라 영원히 True → ⑤퀴즈가 active인 동안 마이크를 서버로 안 보냄 → **영구 정지**. 부수적으로 `drain_playback()`도 매번 8초 타임아웃을 다 쓰게 된다.

**세 겹으로 수정**:
1. **근본 원인** — `launcher.py`에 `reset_after_lost_session()` 신설. 재연결 경로(`_SessionExpired`/`ConnectionClosed`) 양쪽에서 `speaking_done.set()` + `turn_seq` 증가에 더해 **`shifter.reset()`으로 잔량을 즉시 0으로** 만든다(어차피 그 발화는 중간에 끊겼다).
2. **안전장치** — 판단 로직을 `core/mic_gate.py`의 순수 함수 `decide_withhold_mic()`로 분리하고(core/idle_watcher.py와 같은 패턴, launcher 없이 테스트 가능) **시간 상한**을 넣었다: 마지막 오디오 청크 도착 후 `MAX_MIC_WITHHOLD_SEC`(3초)이 지나면 어떤 신호가 잘못 남아있든 무조건 마이크를 연다. 이 게이트는 틀렸을 때 대가가 비대칭적이라(안 막으면 끼어들기 정도, 잘못 막으면 대화 불가) 근본 수정과 별개로 상한이 반드시 필요하다.
3. **같이 발견한 별개 버그** — `inject_turn()`이 함수 진입 시점의 세션 객체를 붙잡은 채 `speaking_done.wait()`/`drain_playback()`을 기다려서, 그 사이 재연결되면 **이미 죽은 세션으로 전송**해 예외가 났다. 그 예외는 호출한 태스크(지연된 거절 대사, 정답 공개 전환)를 통째로 죽여 퀴즈를 멈추게 한다. 대기 후 세션을 다시 읽도록 고치고, 전송 실패는 경고만 남기고 삼키도록 했다(주입 하나 실패로 퀴즈 전체가 멈추면 안 됨). 짜증유발의 10~12초 스톨 중에 GoAway가 오면 정확히 이 경로를 탄다.

**로그에서 확인된 정상 동작**: 짜증유발이 포기 요청에 "화면에 적힌 정답은 '맹구'라고 하네요"라고 **사용자 추측을 정답인 양 지어내 말했지만**, 42단계의 음소거 게이트가 그 오디오를 막았고 곧이어 주입된 진짜 정답('머그컵')만 들렸다 — 음소거가 실제로 값을 하고 있다는 증거(전사에는 남으므로 나중에 추적 가능).

**검증**: 오프라인 9개 스위트 전부 통과(`test_mic_gate.py` 신규 7케이스 — 특히 "재생 잔량 신호가 True로 박혀 있어도 상한이 지나면 마이크가 열린다" 회귀 방지, `test_voice_shift.py`에 "flush 없이 남은 잔량을 reset이 0으로 만든다" 2케이스 추가). **실물 확인 필요**: ①긴 세션에서 GoAway 재연결 후에도 대화가 이어지는지(이번 사고의 직접 재현 조건) ②퀴즈 중 barge-in 차단이 여전히 동작하는지 ③짜증유발 5문제 완주.

### 42단계 — 하찮미 실물 테스트: 퀴즈가 통째로 멈추던 막다른 길 + 퀴즈 중 barge-in 차단 + 2·3번 모드 시작 안내 (2026-08-10)

41단계 적용 후 사용자 평가 "지금 너무 잘돼" — 끊김/지연 문제는 해결된 것으로 확인. 이어서 2번(하찮미)을 실물로 돌리다 새 문제가 나왔고, 추가 요구 2건을 함께 반영했다. **1번(척척박사)은 더 손대지 않기로 확정.**

**(1) 하찮미 3번째 문제에서 퀴즈가 영영 멈춤 — 상태 기계의 막다른 길.** 실물 로그: 모델이 "저는 노란 부분이 치즈 같아요! 정답을 확인해볼까요?"까지 말해놓고 **`submit_guess(speaker="robot")`을 호출하지 않았다**(대신 `set_emotion(tender)`만 부름). 하찮미 모드는 그 호출이 있어야만 채점/전진하므로 `pending_user_guess`가 채워진 채 영구 정지했고, 참가자가 "정답 보여줘야지, 모티야"라고 해도 복구 경로가 없었다(모델은 "판정을 받아야 해서 조금만 기다려주세요"라며 오히려 사용자를 기다리게 함).
- 30단계(척척박사가 submit_guess를 건너뛴 사고)와 같은 부류다 — 그때는 페르소나를 강화해서 대응했지만, **모델이 툴을 부르는 걸 100% 보장할 수 없다는 게 이번에 다시 증명됐다.** 그래서 이번엔 프롬프트가 아니라 구조로 막는다: 상태 기계에 막다른 길이 없도록 탈출구를 만든다.
- `core/quiz_tools.py`에 감시 태스크 신설 — 하찮미가 로봇 추측을 기다리는 상태가 되면(`resolve_user_guess`/`request_hint` 두 진입 경로 모두) 턴 종료 + `ROBOT_GUESS_GRACE_SEC`(2초)를 기다려보고, 호출이 안 오면 **한 번 재촉**(히든 턴으로 "지금 즉시 submit_guess(speaker='robot')를 호출하세요", 그 턴은 음소거)하고, 그래도 안 오면 `core/quiz_state.py`의 신규 `resolve_robot_missing_guess()`로 **사용자 답만으로 채점하고 전진**시킨다.
- 복구 시 로봇 추측은 **억지로 지어내지 않고 비워둔다**(`robot_guess_text=None`) — 연구 로그에서 "이 문항은 로봇 추측이 누락됐다"를 나중에 구분할 수 있어야 하기 때문. 반응 텍스트도 로봇 추측을 주장하지 않는 3갈래(사용자 정답/오답/모름)로 따로 작성.
- 회귀 테스트 5건 추가: staging 확인, 재촉 1회 발생, 멈추지 않고 전진, 복구 결과의 필드 값(사용자 답 유지 + 로봇 추측 None), 화면 reveal 정상 표시, 그리고 **정상 경로(로봇이 제때 호출)에서는 감시가 조용히 물러나는지**(억지 채점이 끼어들면 안 됨).

**(2) 퀴즈 중 barge-in 차단(사용자 요청).** barge-in은 Live API 서버의 자동 VAD가 처리하고 그 설정은 연결 시점에 고정돼 세션 도중 못 바꾼다(system_instruction/tools와 같은 제약) — 그래서 서버 설정을 끄는 대신 **로봇이 말하는 동안 마이크 오디오를 서버로 안 보내는** 방식으로 구현했다(`launcher.py`의 `should_withhold_mic()`). "말하는 중"은 생성 중(`speaking_done`)이거나 아직 스피커로 흘러나오는 중(`pending_sec`)인 경우 둘 다 포함 — 재생이 생성보다 최대 1초 이상 뒤처지므로 둘 다 봐야 발화 끝까지 보호된다. **퀴즈 밖 일반 대화의 barge-in은 그대로 유지**(v3가 AEC까지 도입한 이유가 barge-in이라 절대 건드리면 안 됨). `.env`의 `QUIZ_DISABLE_BARGE_IN=false`로 되돌릴 수 있다. 트레이드오프: 로봇이 말을 마치기 전에 참가자가 답하기 시작하면 그 앞부분이 서버에 안 들어간다(퀴즈에서는 의도된 동작).

**(3) 2·3번 모드 시작 시 "저도 정답을 모릅니다" 명시(사용자 요청).** `choose_mode()`가 imperfect/annoying에 대해 첫 문제를 보여주기 전에 그 사실을 사용자에게 밝히도록 지시문 변경. 조작 점검 문항("로봇이 정답을 모르는 상태로 함께 풀었다")과 직결되는 안내라 참가자가 처음부터 전제를 알고 들어가야 한다. 모드 캐릭터는 유지 — 하찮미는 밝고 친근하게("함께 맞춰봐요!"), 짜증유발은 담백하고 무뚝뚝하게. 척척박사 경로는 무수정.

**검증**: 오프라인 8개 스위트 전부 통과. **실물 확인 필요**: ①하찮미 5문제를 끝까지 진행(중간에 멈추지 않는지, 재촉/복구 경고가 뜨면 어느 문제였는지) ②퀴즈 중 로봇 말을 끊으려 해도 안 끊기는지, 반대로 답변 앞부분이 잘리지는 않는지 ③2·3번 시작 안내가 자연스럽게 나오는지 ④3번(짜증유발) 전체 회귀.

### 41단계 — 40단계 후속 실물 테스트: 끊김의 진짜 원인 확정 + 응답 지연 회귀 수정 (2026-08-10)

40단계 적용 후 척척박사 재테스트. 질문 중복은 해결됐으나(로그상 "이 물건은 무엇일까요?"가 한 번만 나옴) 두 가지가 남았다: ①여전히 끊기는데 **항상 "이 물건은 무엇일까요?"의 '무엇'과 '일까요?' 사이** ②응답 생성이 눈에 띄게 느려짐. 이번엔 추측 대신 전부 재현·측정했다.

**끊김 — 원인 확정: 마지막 자투리를 turn_complete까지 붙잡고 있던 것.** `VoiceShifter`는 `buffer_ms`가 다 차야 블록을 내보내고, 마지막 자투리는 `flush()`가 올 때까지 붙잡는다. 그런데 `flush()`는 서버의 `turn_complete`를 받아야 불리고, Live API는 마지막 오디오 청크보다 늦게 turn_complete를 보낸다 — 그 지연 동안 스피커가 계속 재생하니 쿠션이 바닥나면 **정확히 마지막 블록 경계**에서 무음이 들어간다. 발화가 짧을수록 그 경계가 문장 한복판이고, "이 물건은 무엇일까요?"는 매번 토씨 하나 안 틀리고 같으니 늘 같은 음절에서 끊긴다. 재현: turn_complete 500ms 지연 시 1.4초 발화의 46% 지점에 520ms, 1.9초 발화의 55% 지점에 620ms 무음. 수정: `VOICE_SHIFT_IDLE_FLUSH_MS`(150ms) 신설 — 입력이 끊기면 flush를 기다리지 않고 자투리를 즉시 방출한다(`is_flush=False`라 오버랩/크로스페이드 상태는 유지되므로 뒤에 오디오가 더 와도 이어붙는다). 검증 후 지연 0/300/500/800ms 전부에서 무음 0ms.
- **측정으로 기각한 가설들**(헛다리 재방지용 기록): pyworld GIL 점유 — 기각(감시 스레드 최악 지연이 유휴 기준선과 동일). 짧은 자투리의 F0 분석 실패 — 기각(끝부분 F0 오차가 통째 변조한 이상적 출력과 동일). 크로스페이드 위상 상쇄 — 경계와 비경계 구간이 구분 안 됨. 일반적 재생 부족 — turn_complete가 제때 오면 끊김 0곳.
- **유휴 방출을 넣으면서 실제 크래시를 하나 만들었다가 잡음**: 자투리가 오버랩(120ms)보다 짧으면 붙잡아둘 출력 꼬리가 overlap 길이에 못 미쳐 다음 크로스페이드가 shape 불일치로 터지고, **변조 스레드가 죽어 그 세션 내내 로봇이 벙어리**가 된다(재현함). 호출부 가드(`prev_in_tail + buf > overlap`) + 워커 루프 전체에 예외 방어망(어떤 예외든 그 조각만 버리고 계속 — 이 스레드가 죽는 건 실험 중 최악의 실패라)을 같이 넣음. 회귀 테스트 2건 추가.

**언더런 카운터가 실제 무음의 1/5만 보고하고 있었음.** 40단계에서 쿠션을 넣으면서 "priming 중 무음은 의도된 것"이라고 제외했는데, 버퍼가 마르면 쿠션을 다시 채우느라 그 뒤로도 계속 무음이 나가고 **그게 무음의 대부분**이었다. 실측으로 진짜 520ms 무음을 "2회/100ms"로 보고 — 실물 로그의 "16회/638ms"도 실제보다 한참 작은 숫자였던 것. 이제 "버퍼가 빈 순간부터 재생이 실제 재개될 때까지"를 통째로 재고, 발화 사이의 정상 침묵은 `MAX_MIDSPEECH_GAP_SEC`(1.5초) 초과로 걸러낸다. 발화 끝마다 1회씩 세던 양성 오탐도 같이 사라짐.

**응답 지연 — 40단계가 만든 회귀.** 실물 로그에서 모델이 `_HOLD_FOR_REVEAL`을 **그대로 낭독**하고 있었다("판정했습니다. 이 턴에서는 그 어떤 말도 하지 마세요. 화면이 바뀌는 대로..."). 음성은 40단계의 mute 게이트가 막아주지만 생성 시간은 출력 길이에 비례하므로, 60자짜리 안내문을 읽는 4~5초가 통째로 죽은 시간이 됐다(정답 공개는 그 턴이 끝나야 시작). 40단계 이전엔 그 발화가 최소한 들리기라도 했다. 수정: `_HOLD_FOR_REVEAL`을 `"[OK] 침묵. 대기."`로 축약 — 사람이 읽을 산문이 아니라 기계 상태 토큰처럼 보이게 하고, 낭독되더라도 1초 안에 끝나게.

**지연이 만든 2차 사고 — 문항 또 하나 손실.** 로그에서: 판정 후 아무 반응이 없자 참가자가 답을 다시 말했고("나무인 거 같은데" → "나무인 거 같다고"), 그 반복 발화가 **다음 문항(빨래집게)의 답으로 채점되고 소모**됐다 — 참가자는 그 사진을 보지도 못했다. 40단계의 `user_spoke` 가드는 "사용자가 말했는가"만 보므로 이건 못 막는다(실제로 말했으니까). `_push_question_or_hide()`가 새 사진을 띄울 때 `user_spoke`를 버리도록 수정 — 사진이 뜨기 전에 한 말은 그 문제의 답이 될 수 없다.

**검증**: 오프라인 전부 통과 — `test_quiz_tools.py`(회귀 1건 추가), `test_voice_shift.py`(유휴 방출 + 오버랩 미만 자투리 크래시, 3건 추가), `test_playout_margin.py`(turn_complete 지연 4종, 파형에서 무음 위치를 직접 찾는 방식), 나머지 5개 스위트 회귀 없음. **실물 확인 필요**: ①"무엇일까요?" 끊김이 실제로 사라졌는지 ②답한 뒤 정답까지의 대기가 짧아졌는지 ③세션 종료 시 언더런 경고(이제 정직한 숫자) ④2·3번 모드 회귀.

### 40단계 — 척척박사 실물 테스트 1회에서 나온 버그 3건: 질문 중복, 유령 판정, 발화 중간 끊김 (2026-08-10)

사용자가 1번(척척박사) 모드만 실물로 돌려보고 대화 로그 전체를 제보. 표면적으로는 2건("이 물건은 무엇일까요?"가 두 번 나옴, 목소리가 중간에 먹힘)이었지만, 로그를 시간순으로 재구성하니 연구 데이터가 조용히 망가지는 3번째 문제가 같이 있었다.

**(1) 같은 질문을 두 번 하던 문제.** 원인은 정답 반응을 시키는 히든 턴에 "여기까지만 말하라"는 제약이 없었던 것 — 모델이 반응을 말한 김에 "다음 문제입니다. 이 물건은 무엇일까요?"까지 이어서 말해버렸다. 그런데 그 시점 화면은 아직 정답 공개 사진이고 다음 문제는 `REVEAL_HOLD_SEC`(4초) 뒤에야 뜬다 → 참가자는 보지도 못한 문제를 질문받고, 4초 뒤 화면이 바뀌면서 `next_question_prompt()`가 같은 질문을 또 한다. `core/quiz_tools.py`에 `_REVEAL_SPEECH_SUFFIX`를 신설해 모든 반응 주입에 "다음 문제를 미리 묻지 말라"는 제약을 항상 붙인다.

**(2) 판정 턴 침묵을 파이썬이 강제.** 39단계에서 판정 툴의 응답을 침묵 지시(`_HOLD_FOR_REVEAL`)로 바꿨는데, 실물 로그에서 모델이 그 지시를 어기고 그냥 말하는 경우가 여러 번 있었다("이 물건은 무엇일까요?", "가죽이라고 추측하시는군요."). 지시를 지켜주길 기대하는 대신 `launcher.py`에 `mute_speech` 게이트를 두고, 침묵 지시를 내린 턴 동안 도착하는 오디오를 아예 재생하지 않는다(`core/quiz_tools.py`의 `_mute_until_turn_ends()`가 켜고, 그 턴이 끝나면 자동 해제 + 20초 상한 안전장치). 텍스트 로그에는 그대로 남아 모델이 뭘 말하려 했는지는 추적 가능. 39단계 철학("모델의 턴 배치를 신뢰하지 않고 파이썬이 통제")의 발화 버전.

**(3) 유령 `submit_guess` — 연구 데이터 무결성 문제(제보에 없었으나 로그에서 발견).** 사용자가 아무 말도 안 했는데 모델이 `guess_text="사용자의 말을 기다리는 중"`, `"사용자의 대답을 기다리는 중"`으로 판정을 요청했고, 그때마다 그 문항이 **오답으로 채점되고 소모**됐다 — 이 세션에서 5문항 중 2문항이 참가자가 보지도 못한 채 오답으로 기록됐다. N=30 실험이면 그대로 결과 오염. `launcher.py`가 `input_transcription`에서 `user_spoke` 신호를 올리고, `submit_guess(speaker="user")`는 그 신호 없이는 판정을 거부한다(read-once로 소비 — 같은 발화가 두 번 판정되지 않음). 잘못 막았더라도 사용자가 실제로 답했다면 다음 턴에 다시 호출되므로 손해는 턴 하나뿐. `core/utils.py` 페르소나에도 같은 취지의 금지 문구 추가(이중 방어).

**(4) "말하다 중간에 먹히는" 목소리 — 원인 확정하고 처방을 바꿨다.** 이 증상은 2026-07-28부터 반복 제보됐고 그때마다 `VOICE_SHIFT_BUFFER_MS`를 키워서 대응했다(500 → 700 → 1200ms). **전부 헛다리였다.** 이번엔 추측 대신 실측했다:

- 먼저 의심한 "pyworld가 GIL을 잡아 오디오 콜백이 굶는다" 가설은 **기각** — 감시 스레드 최악 지연이 유휴 기준선(11ms)과 동일했고, `shift_pcm` 비용도 실시간의 24%뿐이라 CPU 자체는 전혀 부족하지 않았다.
- 진짜 원인은 구조였다. 워커는 버퍼가 다 차야 변조를 시작하므로, 서버가 오디오를 실시간 속도로 보내는 구간에서 **블록 N의 변조 완료 시각과 재생 필요 시각이 정확히 같다**(둘 다 `N*B + P`). 즉 지터 여유가 **항상 0**이고, 이 결론은 버퍼 크기 B에도 처리시간 P에도 의존하지 않는다 — 버퍼를 키우면 발화 시작 지연만 늘 뿐 여유는 1ms도 안 생긴다. 세 번의 버퍼 상향이 전부 효과가 없었던 이유가 이것.
- 처방: 여유는 **재생 시작을 늦춰서** 만든다. `media/audio_manager.py`의 `Speaker`에 플레이아웃 지터 버퍼(`PLAYOUT_PRIME_MS`=600ms, 아주 짧은 발화용 `PLAYOUT_PRIME_TIMEOUT_SEC`=0.5s 탈출구)를 넣어, 버퍼가 빈 상태에서 새 발화가 시작되면 쿠션을 먼저 채우고 재생한다. 재생 중 버퍼가 마르면 다시 쿠션을 채운다. 내부 큐를 `queue.Queue`+leftover에서 락으로 보호되는 단일 `bytearray`로 바꿔 잔량을 O(1)로 알 수 있게 함.
- 동시에 `VOICE_SHIFT_BUFFER_MS`를 **1200 → 500(원래 값)으로 되돌렸다** — 블록이 작을수록 발화 시작이 빠르고 한 블록이 늦어졌을 때 손해도 작다. 경계 아티팩트는 32단계의 크로스페이드가 이미 따로 책임진다. 순 발화 시작 지연은 1488ms → 약 1150ms로 오히려 **줄었는데** 지터 여유는 0 → 600ms로 생겼다.
- 검증: `scripts/test_playout_margin.py` 신규 — `sounddevice.OutputStream`을 타이머 기반 가짜 장치로 바꿔치기해 실제 장치 없이 실시간 도착을 재현하고 언더런을 센다. 6초 발화 기준 **이전 설정 100ms(2회) → 현재 설정 0ms(0회)**, CPU 경합을 걸어도 동일. 한가한 개발 PC에서조차 이전 설정에 무음이 끼었다는 게 "여유 0" 구조의 증거.
- 부수 수정: `launcher.py`의 언더런 경고가 "`VOICE_SHIFT_BUFFER_MS`를 늘려보라"는 **틀린 처방**을 안내하고 있어 `PLAYOUT_PRIME_MS` 쪽으로 고쳤다. 또 턴 사이 대기가 `POST_SPEECH_DRAIN_SEC`(버퍼의 2배 = 2.4초) 고정 상수였는데, 이제 `Speaker.pending_sec`/`VoiceShifter.pending_sec`로 남은 재생량을 실제로 물어보는 `drain_playback()`으로 교체 — 정확할 뿐 아니라 짧은 발화 뒤 죽은 시간이 사라져 퀴즈 진행이 전반적으로 빨라진다.

**검증 상태**: 오프라인 전부 통과 — `test_quiz_tools.py`(64케이스, 위 3건 회귀 테스트 9개 추가), `test_quiz_state.py`, `test_quiz_bank.py`, `test_emotion_tools.py`, `test_idle_watcher.py`, `test_voice_shift.py`, 신규 `test_playout_margin.py`. **실물 확인 필요**: ①질문이 한 번만 나오는지 ②판정 직후 로봇이 조용히 있다가 사진이 바뀐 뒤에 정답을 말하는지 ③발화 중간 끊김이 실제로 사라졌는지(세션 종료 시 언더런 경고가 안 뜨는지) ④발화 시작이 느려졌다는 느낌은 없는지 ⑤2·3번 모드도 회귀 없는지(이번 수정은 세 모드 공통 경로를 건드림).
...
**① 청크 경계 톤 불연속("말...씀" 끊김, 14단계에서 가설 세우고 보류).** `VoiceShifter`가 1.2초 청크를 서로 독립적으로 pyworld 처리해서, 단어가 경계에 걸리면 앞/뒤의 F0/스펙트럼 추정이 어긋나 톤이 뚝 끊겼다. **수정**: 오버랩-크로스페이드 도입 — 각 청크 앞에 직전 청크의 입력 꼬리(`VOICE_SHIFT_OVERLAP_MS`, 기본 120ms)를 이어붙여 분석하고, 겹치는 구간의 출력은 직전에 붙잡아둔 출력 꼬리와 선형 크로스페이드(rint 반올림 — astype 버림이면 항등 변환에도 ±1 오차가 남는 걸 테스트로 발견)로 섞는다. 부수 효과로 **턴 마지막 짧은 꼬리(MIN_SHIFT_SAMPLES 미만)가 변조 없이 원본 톤으로 재생되던 문제(14단계에서 "감수하고 넘어가기로" 했던 원본/변환본 톤 차이)도 함께 해결** — flush 시 꼬리가 항상 직전 120ms 문맥과 합쳐져 변조된다. 트레이드오프: 재생 지연 +120ms, pyworld 처리량 +10%(둘 다 `POST_SPEECH_DRAIN_SEC` 2배 마진 안).

**② barge-in 후 스테일 오디오 누출(28단계 리뷰에서 발견, 보류).** `reset()`이 큐 뒤에 마커로 줄을 서는 구조라, 이미 큐에 쌓였거나 pyworld 처리 중이던 오디오가 `stop_immediately()` 이후에도 최대 1.2초 뒤늦게 새어나올 수 있었다. **수정**: 세대(generation) 번호 도입 — `reset()`이 세대를 올리고, `feed()`가 데이터에 그 시점 세대를 태깅, 워커는 큐에서 꺼낼 때와 (오래 걸리는) 처리를 마치고 재생 직전 두 번 세대를 검사해 이전 세대 오디오를 폐기한다.

**검증**: `scripts/test_voice_shift.py` 신규 — (1) shift_pcm을 항등 함수로 패치하면 출력이 입력과 바이트 단위로 동일해야 함(오버랩 정렬의 가장 강한 증거, 실제로 이 테스트가 크로스페이드의 버림 오차 ±1을 잡아냄), (2) 느린 변환 함수로 패치해 reset 시점에 처리 중이던 것과 큐 대기 중이던 것 둘 다 재생 안 되는지, (3) 실제 pyworld로 사인파 변조 — 길이 보존 + 청크 경계 최대 점프가 내부 수준(3244)으로 유지됨(기존 독립 청크 방식은 같은 조건에서 4101로 초과함을 별도 측정으로 확인). 1초 오디오 변조 0.36초(실시간 0.36x, 개발 PC 기준). 기존 오프라인 테스트 전부 재통과. **실물 로봇에서 귀로 확인 필요** — 특히 "편하게 말씀해주세요" 같은 문장이 끊기지 않는지, barge-in 직후 잔여 음성이 안 나오는지, 언더런 로그가 늘지 않는지(오버랩만큼 CPU를 더 쓰므로).

### 31단계 — Ctrl+C 종료 시 퀴즈 결과·대화록이 통째로 유실되던 문제 수정 (2026-08-07)

전체 코드베이스 재검사에서 발견. `run_conversation()`이 `quiz_log_out.extend(...)`와 `return session_history`를 **정상 종료 경로에서만** 실행하고 있어서, 참가자가 작별 인사([대화종료]) 없이 자리를 뜨거나 어떤 이유로든 실험자가 Ctrl+C로 세션을 끊으면 `KeyboardInterrupt`/`CancelledError`가 그 지점을 건너뛰어 그 참가자의 퀴즈 결과·대화록·결과지가 전부 사라졌다(28단계에서 고친 "툴 예외로 인한 유실"과 같은 클래스인데, 종료 경로가 달라 그때 안 잡혔던 것). N=30 실험에서 충분히 일어날 수 있는 시나리오.

**수정**: 결과를 "끝나고 복사"하는 대신 "시작 시점에 참조를 넘겨두는" 방식으로 전환 — `name_state`/`session_holder`와 같은 가변 컨테이너 패턴.
- `quiz_log_out: list` 파라미터를 `quiz_session_out: dict`({"session": None})로 교체 — `make_quiz_tools()` 직후 QuizSession 참조를 채워 넣고, `main()`의 `finally`가 종료 방식과 무관하게 `export_log()`를 직접 뽑아 `save_quiz_results()`에 넘긴다.
- `history_out: list` 파라미터 신설 — `session_history`를 호출자가 준 리스트에 직접 누적해, 세션이 끊겨도 그때까지의 턴 기록이 호출자에게 남는다.

**검증**: `py_compile` 통과, 기존 오프라인 테스트 전부 재통과(run_conversation은 launcher.py 전용이라 다른 스크립트 영향 없음). 실제 Ctrl+C 경로는 로봇+Live API가 필요해 다음 실물 가동 때 확인 권장(대화 중 Ctrl+C → user_result/에 대화록·퀴즈 파일이 남는지).

### 30단계 — 척척박사 모드: submit_guess 건너뛰고 말로만 답해 화면이 안 넘어가는 사고 + 톤 이탈 (2026-07-31)

29단계 직후 실물 재확인 중 하찮미(2번)는 대체로 잘 됐다는 확인을 받았고(가끔 타이밍이 느리거나 빠른 잔차는 남아있음, 아래 "남은 것" 참고), 대신 척척박사(1번) 모드에서 새 문제가 드러남. 사용자가 "누가 알려줘"라고 하자 모델이 `submit_guess`/`request_hint` 어느 것도 호출하지 않고 곧장 "후후, 이번 정답은 바로 '칫솔'입니다! 매일 아침저녁으로 만나는 물건인데, 확대해 보니까 색다르죠?"처럼 말로만 답해버림 — `_record_and_advance()`가 전혀 실행되지 않아 (1) 이 응답이 연구 로그에 전혀 안 남고 (2) 화면이 다음 문제로 영영 안 넘어감(18단계 "파프리카" 사고와 같은 클래스: 모델이 툴 호출 없이 스스로 답을 말해버리는 LLM 신뢰성 한계). 이어서 "그래, 다음으로 넘어가죠"에 화면이 안 뜬 채로 "네, 화면에 다음 문제가 떴습니다!"라고 잘못 말하고 `play_gesture(shy)`(부끄부끄)까지 재생 — 척척박사 캐릭터에 안 맞는 하찮미st 제스처. 사용자 진단: "너무 감정을 담아서 이야기하는거 같아... 딱딱하게 문제 시작합니다. 정답은 ~입니다 로만 말해야하는데".

원인은 all_knowing 모드가 `choose_mode()` 시점에 정답을 시스템 컨텍스트에 평문으로 미리 쥐여줘서(척척박사가 "이미 알고 있다"는 캐릭터를 위한 의도된 설계) 모델이 툴을 거치지 않고도 답을 말할 유혹이 구조적으로 존재한다는 점 — imperfect/annoying은 정답을 모르는 채로 시작해 툴 호출이 정답을 아는 유일한 경로라 이 문제가 구조적으로 안 생긴다. 코드 아키텍처를 바꾸는 대신(위험 대비 실익이 낮다고 판단, 남은 로드맵도 실물 검증 위주라 큰 리팩터는 보류) `core/utils.py`의 퀴즈 지시 블록을 프롬프트 레벨에서 강화:
1. "곧바로 submit_guess를 호출하세요" 규칙에 "정답 알려줘"/"누가 알려줘"류 표현을 명시적 예시로 추가하고, "척척박사는 이 규칙의 예외가 아니다"와 "건너뛰면 화면이 절대 안 넘어간다"는 구체적 결과를 명시(18단계 "파프리카" 사고 대응 문구와 같은 패턴 — 실제 사고를 반례로 인용).
2. 척척박사 전용 톤/제스처 규칙 신설: 정답은 "정답은 OO입니다." 정도로 짧고 사무적으로만, 감탄사·너스레·부연설명 금지(이번 "후후... 색다르죠?" 사고를 반례로 인용). 화면 이상 상황에서도 당황/사과 대신 침착하게. 부끄부끄(`play_gesture(name="shy")`) 같은 감정적 제스처는 하찮미 캐릭터 전용이니 척척박사 중엔 쓰지 말고, 필요하면 고개 끄덕임 정도만.

**검증**: `core/utils.py`는 순수 프롬프트 텍스트라 오프라인 테스트가 없음(기존 관례와 동일) — `py_compile` 통과만 확인, 문구 자체는 검토로만 확인. 완전 예방은 LLM 신뢰성 한계상 불가능하다는 점은 18단계 때와 동일 — 재발 시 이번 반례 문구를 더 강화할 것. **아직 실물 미확인.**

**하찮미(2번) 남은 잔차(2026-07-31 사용자 확인)**: 29단계 sync 수정 이후 대체로 잘 되지만 "정답 나오는 타이밍이 가끔 느리거나 다음 문제로 넘어가는 타이밍이 빠른 경우"가 남아있음 — 심각도 낮음 판단, 우선 척척박사 문제를 먼저 처리하고 다음에 필요하면 재검토(스트리밍이라 매 턴 실제 발화 길이가 달라 `speaking_done` 기반 동기화에도 약간의 잔차가 있을 수 있음).

### 29단계 — 하찮미 정답 공개 타이밍 sync 버그 수정 + 오답 반응을 하찮미답게 재작성 (2026-07-31)

28단계 이후 실사용(2번 하찮미 모드)에서 사용자가 두 가지를 지적: (1) 사용자가 답을 말한 뒤 로봇이 자기 추측을 말하기도 전에 정답 공개 화면이 먼저 뜨고, 로봇이 정답을 다 말할 즈음엔 이미 다음 문제로 화면이 넘어가 있어 참가자가 당황할 만함. (2) 로봇이 틀렸을 때 반응("아쉽다", "부럽다")이 밋밋한 실망으로만 들려서, "로봇의 실수가 오히려 귀엽게 느껴졌다"를 증명해야 하는 하찮미 설계 철학이 잘 안 살아남.

**① sync 버그 원인**: `core/quiz_tools.py`의 `submit_guess()`가 `resolve_robot_guess()` 등의 툴 콜백이 반환되는 즉시(=모델이 함수 호출을 결정한 순간) `_push_reveal()`로 정답 사진을 UI에 밀어넣고 `REVEAL_HOLD_SEC`(4초) 카운트다운을 시작시키고 있었다. 하지만 Live API의 함수 호출은 모델이 아직 텍스트/오디오를 스트리밍하는 도중에도 도착할 수 있고(서버 쪽 생성이 로컬 재생 속도보다 빠름 + `VoiceShifter`가 최대 1200ms 버퍼링), 함수 응답으로 받은 정답 공개 대사는 그 이후에나 발화되기 시작한다 — 그래서 화면이 로봇의 실제 발화보다 항상 먼저 바뀌었다.

**수정**: launcher.py가 이미 `inject_turn()`에 쓰던 `speaking_done`(asyncio.Event — 로봇이 지금 발화 중이 아닐 때만 set) 신호를 `make_quiz_tools()`에 새 인자로 전달받도록 확장. `_push_reveal()` + `REVEAL_HOLD_SEC` 타이머를 하나의 지연 태스크(`_delayed_reveal_and_advance`)로 합쳐, 툴 호출 시점이 아니라 `speaking_done.wait()`(+ `POST_SPEECH_DRAIN_SEC` 드레인)로 **이번 턴의 발화가 실제로 다 끝난 뒤**에만 정답 사진을 띄우고, 그 시점부터 4초를 센 뒤 다음 문제로 넘어간다. `POST_SPEECH_DRAIN_SEC` 상수는 기존에 `launcher.py`에만 있었는데 `core/quiz_tools.py`도 같은 값을 써야 해서 `media/voice_shift.py`로 옮겨 두 파일이 공유하도록 함(단일 출처). `scripts/test_quiz_tools.py`/`scripts/test_quiz_live.py`도 새 `speaking_done` 인자에 맞춰 갱신, 정답 공개가 실제로 `speaking_done`을 기다리는지 확인하는 회귀 테스트(로봇이 아직 말하는 중이면 reveal이 안 뜨다가 발화가 끝나야 뜨는지)를 새로 추가.

**② 하찮미 오답 반응 재작성**: `core/quiz_state.py`의 `resolve_robot_guess()`에서 로봇이 틀렸을 때의 세 가지 반응(단독 오답/사용자만 맞음/둘 다 틀림)을 "아쉽다"/"부럽다" 같은 밋밋한 실망 톤에서, 자신만만했다가 당황하고 스스로도 어이없어하며 웃어넘기는 톤으로 재작성 — 실수 자체를 창피해하기보다 재미있어하는 태도를 명시해 "귀여운 무능함"이 더 드러나도록 함. `scripts/test_quiz_state.py`의 관련 assertion 문자열(`"부럽다"`/`"둘 다 틀렸네요"`/`"아쉽다"`)도 새 문구에 맞춰 갱신.

**검증**: `scripts/test_quiz_state.py`/`scripts/test_quiz_tools.py` 전부 재통과(새 회귀 테스트 포함). `launcher.py`/`core/quiz_tools.py` `py_compile` 통과. **아직 실물 미확인** — 다음 로봇 연결 때 sync가 실제로 맞는지, 새 오답 반응 톤이 자연스럽게 들리는지 확인 필요.
