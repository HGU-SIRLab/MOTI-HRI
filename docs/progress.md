...
**검증**: 신규 `scripts/test_trust_notice.py`(20케이스) — 문장/판정/오탐 방지, 시작 안내 주입·중복 방지·2회 상한, 퀴즈 중 미주입, 마무리 안내 3경로(즉시 종료 / 재요청 후 발화 / 재요청에도 무응답), 실험 모드에서도 지시가 남는지. 오프라인 11개 스위트 전부 통과. **실물 미확인** — 다음 로봇 가동 때 확인할 것: 이름을 말한 직후 안내가 나오는지, 그때 콘솔에 `🔒 프라이버시 안내 발화 확인`이 뜨는지, 작별 인사에 마무리 안내가 포함되는지(빠지면 `↩️` 재요청 로그), 세션 종료 후 `session_meta.json`의 두 플래그가 true인지.
...
**검증**: `scripts/test_report.py`를 새 구조에 맞춰 재작성해 통과 — 잘못된 `PARTICIPANT_ID` 거부(빈 값/경로 구분자), 한 폴더에 6개 산출물이 모이는지, 이름 없이도 대화록이 저장되는지, 빈 퀴즈 로그가 폴더를 안 만드는지, 메타에 모드 순서가 남는지. 이 테스트는 자기가 만든 `__테스트참가자__` 폴더만 지우고 `user_result/`의 다른 것은 건드리지 않는다. **실물 미확인** — 다음 로봇 가동 때 콘솔의 `🧑 이번 참가자:` / `📁 이번 세션 저장 위치:` 출력과, 세션 종료 후 실제 폴더 내용을 확인할 것.
