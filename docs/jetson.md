...
- **CPU 경합 중 오디오 끊김 — Xavier NX 실물 미해결 (2026-08-26).**
  `scripts/test_playout_margin.py`가 이 보드에서 "CPU 경합" 시나리오(카메라 추적 +
  모터 통신 + 표정 UI가 동시에 도는 상황을 흉내낸 4개 스레드 부하)에서 계속 실패한다
  (한가할 때는 통과, 부하 걸리면 700~2500ms 무음 — 재현할 때마다 편차가 큼). 원인은
  MODE_10W_DESKTOP(기본값, 4코어 @ 최대 1.9GHz)에서만 여유가 있고, 6코어 모드로
  바꾸면 코어당 클럭이 1.42GHz로 낮아져 오히려 더 나빠진다는 것까지는 확인했다
  (아래 onnxruntime 문제와 정반대 요구 — 8번 항목이 여기서 이미 그 트레이드오프를
  기록하고 있었다). **테스트의 부하 시뮬레이션(FFT 스핀 스레드 4개)이 실제 카메라
  루프·모터 통신보다 훨씬 공격적일 수 있어, 이게 진짜 문제인지 테스트가 과도하게
  비관적인 것인지는 실물에서 카메라+모터+표정UI를 동시에 띄워보기 전에는 알 수 없다.**
  실물 검증 시 6번 순서 중 5~7단계(얼굴인식+오디오+전체 실행)를 동시에 띄운 상태로
  대화가 끊기는지 직접 들어볼 것. 끊기면 오디오 콜백 스레드에 실시간 우선순위
  (`SCHED_FIFO`) 부여를 다음 시도로 검토.
- **onnxruntime 버전 고정 필요.** `requirements-jetson.txt`의 onnxruntime 섹션 참고 —
  최신 버전은 이 보드에서 core dump가 난다. 반드시 `onnxruntime==1.18.1`.
