...
# 30초. 처음엔 12초로 잡았다가 실물에서 바로 깨졌다 — 2026-09-22 세션 7f067653에서
# 보호 시작 12.0초에 상한이 풀리고 0.35초 뒤 barge-in이 걸려 첫 인사가 잘렸다
# (17:27:57.760 세션 시작 → 17:28:10.107 turn cancelled). 첫 인사는 생성+재생 합쳐
# 12초를 넘는다. 정상 흐름에서는 turn_complete가 오는 즉시 보호가 풀리므로 이 값은
# "turn_complete가 영영 안 올 때"만 쓰이는 순수 안전장치다 — 넉넉해도 된다.
MAX_PROTECT_SEC = 30.0
